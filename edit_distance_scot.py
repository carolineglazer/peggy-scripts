...
for f in sorted(os.listdir('../gen_scot/')):
	if f.endswith('.xml'):
		print('###### '+f+' ######')
		base_peggy = ('../gen_scot/'+f)
		diffs = []
		total = 0
		for i in sorted(os.listdir('../gen_scot/')):
			if i.endswith('.xml'):
				peggy_pair = omr.evaluators.OmrGroundTruthPair(omr=converter.parse(base_peggy),ground=converter.parse('../gen_scot/'+i))
